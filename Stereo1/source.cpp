...
void createViewMatrix(Shader& shader, float ipd, float near, bool rightEye = false) {
	...
	view = glm::lookAt(
		...
		glm::vec3((rightEye ? ipd / 2.0 : -ipd / 2.0), 0.0f, -near),
		glm::vec3(0.0f, 1.0f, 0.0f)
	);
	shader.setUniformMatrix4float("view", view);
}

void createProjectionMatrix(Shader& shader, float ipd, float near = 0.1f, float far=100.0f, float fovDeg=45.0f,  bool rightEye = false) {
	...
	projection = glm::frustum(
		(rightEye ? -(near * aspect * tan(fovby2) + ipd / 2.0f) : -(near * aspect * tan(fovby2) - ipd / 2.0f)), // left
		(rightEye ? (near * aspect * tan(fovby2) - ipd / 2.0f) : (near * aspect * tan(fovby2) + ipd / 2.0f)), // right
		bottom,
		top,
		near,
		far
